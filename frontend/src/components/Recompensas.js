...
const Recompensas = () => {
  ...
  useEffect(() => {
    const carregarRecompensas = async () => {
      ...
      try {
        const res = await fetch(`${API_URL}/api/recompensas/${user._id}`);
  ...
  const handleResgatar = async (index) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/recompensas/obter/${user._id}`,
        {
          method: "POST",
        ...
        <div style={styles.container}>
          ...
          <div style={styles.grid}>
            {recompensasObtidas.map((recompensa, index) => (
              ...
              >
                {recompensa.iconUrl ? (
                  <img src={recompensa.iconUrl} alt={recompensa.titulo} style={styles.icon} />
                ) : (
